...
void	dda_algorythm2(char **map, t_raycaster *rc, t_camera *cam)
{
	...
	while (1)
	{
		...
		else if (rc->side == 1 && map[(int)rc->map_check.y][(int)rc->map_check.x] == 'D')
		{
			...
			// if (rc->side_dist.x + rc->delta_dist.x >= rc->side_dist.y + rc->delta_dist.y / 2)
			// {
			// 	rc->side_dist.y += rc->delta_dist.y / 2;
			// 	break ;
			// }
			...
			// else
			// {
			// 	if (rc->dir.y < 0 && (rc->camera_x > -0.4 && rc->camera_x < 0.4))
			// 		break ;
			// 	else if (rc->dir.y > 0 && (rc->camera_x > -0.4 && rc->camera_x < 0.4))
			// 		break ;
			// 	else if (rc->dir.y == 0)
			// 		break ;
			// }
...
void	raycaster_setup2(t_raycaster *rc, t_camera cam, int screen_x)
{
	...
	// if (cam.pos.x - (int)cam.pos.x < 0.5)
	// 	rc->map_check.x = (int)cam.pos.x;
	// else
	// 	rc->map_check.x = (int)cam.pos.x + 0.5;
	rc->map_check.x = (int)cam.pos.x;

	// if (cam.pos.y - (int)cam.pos.y < 0.5)
		rc->map_check.y = (int)cam.pos.y;
	// else
		// rc->map_check.y = (int)cam.pos.y + 0.5;
	...
	// else
	// 	rc->side_dist.x = (rc->map_check.x + 0.5 - cam.pos.x) * rc->delta_dist.x;
	if (rc->dir.x < 0)
		rc->side_dist.x = (cam.pos.x - rc->map_check.x) * rc->delta_dist.x;
	else
